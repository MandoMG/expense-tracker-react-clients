    ...
    return (
      <View style={commonStyles.listItemWrapper}>
        <View style={commonStyles.listItemSingleLineLeftColumn}>
          <Text style={commonStyles.listItemMainText}>{summaryItem.categoryName}</Text>
        </View>
        <View style={commonStyles.listItemRightColumn}>
          <Text style={commonStyles.listItemMainText}>{TextUtil.formatCurrency(summaryItem.categoryValue)}</Text>
          <View style={commonStyles.flexRow}>
            <Icon name="circle" color={summaryItem.isUnderBudget ? '#14a023' : '#c82d1e'} style={{ marginHorizontal: 5, marginTop: 3 }} solid />
            <Text style={commonStyles.listItemSubText}>{`${TextUtil.formatCurrency(summaryItem.budgetDifference)} ${summaryItem.isUnderBudget ? 'under' : 'over'} budget`}</Text>
          </View>
        </View>
      </View>
    );
      ...
      <View style={BudgetSummaryStyles.budgetSummaryWrapper}>
        ...
        <View style={BudgetSummaryStyles.BudgetSummaryListWrapper}>
          <FlatList
            ...
            renderItem={({ item }) => (
              <BudgetSummaryItem summaryItem={item} />
            )}
          />
        </View>
      </View>
